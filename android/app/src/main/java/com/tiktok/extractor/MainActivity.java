...
public class MainActivity extends AppCompatActivity {
    ...
    private void openInAppBrowserInternal(String url) {
        ...
        mainHandler.post(() -> {
            try {
                ...
                // Copy All Cookies button (NEW v1.0.27)
                TextView copyBtn = new TextView(this);
                copyBtn.setText("Copy All");
                copyBtn.setBackgroundColor(0xCCFFB547);
                copyBtn.setTextColor(0xFF000000);
                copyBtn.setPadding(24, 12, 24, 12);
                copyBtn.setTextSize(14);
                copyBtn.setOnClickListener(v -> copyAllCookies());
                FrameLayout.LayoutParams copyP = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT);
                copyP.gravity = Gravity.TOP | Gravity.LEFT; copyP.topMargin = 110; copyP.leftMargin = 16;
                root.addView(copyBtn, copyP);

                // Deep Extract button (NEW v1.0.27)
                TextView deepBtn = new TextView(this);
                deepBtn.setText("Deep Extract");
                deepBtn.setBackgroundColor(0xCC9C27B0);
                deepBtn.setTextColor(0xFFFFFFFF);
                deepBtn.setPadding(24, 12, 24, 12);
                deepBtn.setTextSize(14);
                deepBtn.setOnClickListener(v -> triggerDeepExtractInBrowser());
                FrameLayout.LayoutParams deepP = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT);
                deepP.gravity = Gravity.TOP | Gravity.LEFT; deepP.topMargin = 160; deepP.leftMargin = 16;
                root.addView(deepBtn, deepP);

                // Save Local button (NEW v1.0.27)
                TextView saveBtn = new TextView(this);
                saveBtn.setText("Save Local");
                saveBtn.setBackgroundColor(0xCC4CAF50);
                saveBtn.setTextColor(0xFFFFFFFF);
                saveBtn.setPadding(24, 12, 24, 12);
                saveBtn.setTextSize(14);
                saveBtn.setOnClickListener(v -> saveSessionLocally());
                FrameLayout.LayoutParams saveP = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT);
                saveP.gravity = Gravity.TOP | Gravity.LEFT; saveP.topMargin = 210; saveP.leftMargin = 16;
                root.addView(saveBtn, saveP);
    ...
    // ════════════════════════════════════════════════════════════════
    //  v1.0.27: Copy All Cookies + Deep Extract + Save Local
    // ════════════════════════════════════════════════════════════════
    ...
    private void copyAllCookies() {
        ...
        CookieManager cm = CookieManager.getInstance();
        String cookies = cm.getCookie("https://www.tiktok.com/");
        if (cookies == null || cookies.isEmpty()) { toast("No cookies found"); return; }
        // Copy to clipboard
        try {
            android.content.ClipboardManager clipboard = (android.content.ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
            android.content.ClipData clip = android.content.ClipData.newPlainText("TikTok Cookies", cookies);
            clipboard.setPrimaryClip(clip);
            toast("Copied " + cookies.length() + " chars (" + parseCookies(cookies).size() + " cookies)");
        } catch (Exception e) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("All Cookies (").append(cookieMap.size()).append("):\n\n");
        ...
        if (display.length() > 4000) display = display.substring(0, 4000) + "...";
        new AlertDialog.Builder(this).setTitle("All Cookies (" + cookieMap.size() + ")").setMessage(display).setPositiveButton("OK", null).show();
    }
    ...
    private void triggerDeepExtractInBrowser() {
        ...
        // Run deep extract on background thread simultaneously
        new Thread(() -> {
            try {
                ...
                if (code == 200) {
                    ...
                    if (d.optBoolean("success")) {
                        String uid = d.optString("unique_id", "unknown");
                        int liveNum = d.optInt("live_number", 0);
                        int files = d.optJSONArray("files_saved") != null ? d.getJSONArray("files_saved").length() : 0;
                        boolean hasWebmssdk = d.optJSONObject("webmssdk") != null && d.getJSONObject("webmssdk").optBoolean("success");
                        mainHandler.post(() -> toast("Deep extract done! @" + uid + " live" + liveNum + " (" + files + " files, webmssdk: " + (hasWebmssdk ? "yes" : "no") + ")"));
                    } else {
                        mainHandler.post(() -> toast("Deep extract failed: " + d.optString("error", "unknown")));
                    }
                } else {
                    mainHandler.post(() -> toast("Deep extract HTTP " + code));
                }
            } catch (Exception e) {
                Log.e(TAG, "deepExtractInBrowser failed", e);
                mainHandler.post(() -> toast("Deep extract error: " + e.getMessage()));
            }
        }).start();
    }
    ...
    private void saveSessionLocally() {
        ...
        CookieManager cm = CookieManager.getInstance();
        String cookies = cm.getCookie("https://www.tiktok.com/");
        if (cookies == null || cookies.isEmpty()) { toast("No cookies to save"); return; }
        Map<String, String> cookieMap = parseCookies(cookies);
        // Save all cookies to SharedPreferences
        SharedPreferences.Editor editor = prefs.edit();
        editor.putString("all_cookies", cookies);
        editor.putString("cookies_saved_at", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(new Date()));
        // Save individual important ones
        editor.putString("sessionid", cookieMap.get("sessionid"));
        editor.putString("ttwid", cookieMap.get("ttwid"));
        editor.putString("msToken", cookieMap.get("msToken"));
        editor.putString("sid_tt", cookieMap.get("sid_tt"));
        editor.putString("sid_guard", cookieMap.get("sid_guard"));
        editor.putString("uid_tt", cookieMap.get("uid_tt"));
        editor.putString("tt_chain_token", cookieMap.get("tt_chain_token"));
        editor.putString("cmpl_token", cookieMap.get("cmpl_token"));
        editor.putInt("cookies_count", cookieMap.size());
        editor.apply();
        toast("Saved " + cookieMap.size() + " cookies locally");
        // Show confirmation
        new AlertDialog.Builder(this)
            .setTitle("Saved Locally")
            .setMessage("Saved " + cookieMap.size() + " cookies to device storage.\n\nSession data persists across app restarts.\n\nUse 'Check Session' to verify.")
