...
class ExtractionResult:
    ...
    # ─── v4.7: Advanced analytics fields ──────────────────────────────────
    top_fans_analysis: Dict[str, Any] = field(default_factory=dict)      # top fans detailed analysis (influencer tiers)
    stream_quality_analysis: Dict[str, Any] = field(default_factory=dict) # FLV/HLS/RTMP stream URLs analysis
    linkmic_analysis: Dict[str, Any] = field(default_factory=dict)       # multi-guest/battle analysis
    ttwid_decoded: Dict[str, Any] = field(default_factory=dict)          # decoded ttwid (device_id + timestamp + signature)
    conversion_analysis: Dict[str, Any] = field(default_factory=dict)    # conversion rates + live efficiency score
    badge_analysis: Dict[str, Any] = field(default_factory=dict)        # owner badge levels + privileges
...
def extract(raw_url: str, timeout: int = 30) -> ExtractionResult:
    ...
    if result:
        ...
        # ─── v4.7: Advanced deep analytics ───
        _analyze_top_fans(result)
        _analyze_stream_quality(result)
        _analyze_linkmic(result)
        _decode_ttwid(result)
        _calculate_conversion_rates(result)
        _analyze_owner_badges(result)
...
# ════════════════════════════════════════════════════════════════════════════
#  v4.7: Advanced Deep Analytics — تحليلات معمّقة من بيانات Webcast الكاملة
# ════════════════════════════════════════════════════════════════════════════
#  هذه المعالجات تستغل البيانات الغنية من 5.json:
#    - top_fans_analyzer: تحليل المعجبين المميزين ومستويات شاراتهم
#    - stream_quality_analyzer: تحليل روابط FLV/HLS/CDN وجودتها
#    - linkmic_analyzer: تحليل البث المتعدد الضيوف (multi-guest)
#    - ttwid_decoder: فك تشفير ttwid (device_id + timestamp + signature)
#    - conversion_calculator: حساب معدلات التحويل الفعلية
#    - badge_analyzer: تحليل شارات المذيع ومستواه
# ════════════════════════════════════════════════════════════════════════════


def _analyze_top_fans(result: ExtractionResult) -> None:
    """يحلل المعجبين المميزين (Top Fans) من بيانات Webcast.

    يستخرج:
      - عدد المعجبين المميزين
      - متوسط متابعي المعجبين (مؤثرين؟)
      - تصنيف المعجبين حسب المتابعين (nano/micro/macro/mega)
      - مستوى الشارات إن توفّرت
      - sec_uid لكل معجب (للتتبع)
      - متوسطfan_tickets
    """
    # المعجبون من donor_rankings أو live.top_fans
    fans = list(result.donor_rankings or [])
    live = result.live or {}
    if live.get("top_fans") and isinstance(live["top_fans"], list):
        # دمج بيانات top_fans من webcast_full_data
        for tf in live["top_fans"]:
            if isinstance(tf, dict):
                user = tf.get("user") or {}
                fans.append({
                    "unique_id": user.get("unique_id") or user.get("nickname"),
                    "user_id": user.get("id") or user.get("user_id"),
                    "sec_uid": user.get("sec_uid"),
                    "nickname": user.get("nickname"),
                    "follower_count": user.get("follower_count"),
                    "avatar": (user.get("avatar_urls") or [None])[0] if user.get("avatar_urls") else None,
                    "rank": tf.get("rank"),
                    "score": tf.get("score"),
                })

    if not fans:
        result.top_fans_analysis = {"available": False, "reason": "No top fans data"}
        result.raw_keys.append("top_fans_analysis_empty")
        return

    follower_counts = [_safe_int(f.get("follower_count")) for f in fans if f.get("follower_count")]
    ...
    # تصنيف المعجبين حسب المتابعين
    classifications = []
    for f in fans:
        fc = _safe_int(f.get("follower_count"))
        if fc >= 1_000_000:
    ...
    # تحليل الشارات (badge levels)
    badge_levels = []
    for f in fans:
        badges = f.get("badges") or f.get("badge_list") or []
        if isinstance(badges, list):
            for b in badges:
                if isinstance(b, dict):
                    level = b.get("level")
                    if level:
                        badge_levels.append(_safe_int(level))

    result.top_fans_analysis = {
        "available": True,
        "total_top_fans": len(fans),
        "fans_detail": [
            {
                "rank": f.get("rank"),
                ...
                "sec_uid": (f.get("sec_uid") or "")[:60],
                "follower_count": f.get("follower_count"),
                "avatar": (f.get("avatar") or "")[:80],
                "fan_ticket_count": f.get("fan_ticket_count") or f.get("score"),
                "classification": classifications[i] if i < len(classifications) else "unknown",
            }
            for i, f in enumerate(fans)
        ],
        "aggregate_stats": {
            "avg_follower_count": round(avg_followers, 0),
            "max_follower_count": max_followers,
            "min_follower_count": min_followers,
            "mega_influencers_count": classifications.count("mega_influencer"),
            "macro_influencers_count": classifications.count("macro_influencer"),
            "micro_influencers_count": classifications.count("micro_influencer"),
            "nano_influencers_count": classifications.count("nano_influencer"),
        },
        "badge_analysis": {
            "badge_levels_detected": badge_levels,
            "max_badge_level": max(badge_levels) if badge_levels else None,
            "avg_badge_level": round(sum(badge_levels) / len(badge_levels), 1) if badge_levels else None,
        },
        "strategic_value": _assess_fans_strategic_value(fans, classifications),
    }
    result.raw_keys.append("top_fans_analysis")
    logger.info(f"👥 Top fans analyzed: {len(fans)} fans, "
                f"avg_followers={avg_followers:.0f}, "
                f"mega={classifications.count('mega_influencer')}, "
                f"macro={classifications.count('macro_influencer')}")


def _assess_fans_strategic_value(fans: List[Dict], classifications: List[str]) -> Dict[str, Any]:
    """يقيّم القيمة الاستراتيجية للمعجبين."""
    mega_count = classifications.count("mega_influencer")
    macro_count = classifications.count("macro_influencer")

    # إجمالي الوصول المحتمل (sum of followers)
    total_reach = sum(_safe_int(f.get("follower_count")) for f in fans)

    return {
        "total_potential_reach": total_reach,
        "has_mega_influencers": mega_count > 0,
        "has_macro_influencers": macro_count > 0,
        "strategic_tier": (
            "tier_1_elite" if mega_count > 0
            else "tier_2_strong" if macro_count > 0
            else "tier_3_normal"
        ),
        "marketing_value": (
            "high — يمكن الوصول لجمهور كبير عبر معجبين مؤثرين"
            if mega_count + macro_count > 0
            else "medium — معجبون عاديون بدون تأثير واسع"
        ),
        "recommendation": (
            "تواصل مع المعجبين المؤثرين للتعاون" if mega_count + macro_count > 0
            else "ركّز على بناء قاعدة معجبين أكبر"
        ),
    }


def _analyze_stream_quality(result: ExtractionResult) -> None:
    """يحلل جودة روابط البث (FLV/HLS/RTMP) وبروتوكولاتها.

    يستخرج:
      - جميع صيغ البث المتاحة (HLS, FLV, RTMP)
      - جودة كل صيغة (HD1, SD1, etc.)
      - خوادم CDN المستخدمة
      - انتهاء صلاحية كل رابط
      - بروتوكول مثالي للتسجيل (FFmpeg command)
      - مستقر أم لا (عدّة CDN = stability)
    """
    live = result.live or {}
    cdn_meta = result.cdn_metadata or {}
    ...
    # جمع كل روابط البث
    stream_urls = {}
    if live.get("stream_urls") and isinstance(live["stream_urls"], dict):
        stream_urls.update(live["stream_urls"])
    if live.get("webcast_stream_urls") and isinstance(live["webcast_stream_urls"], dict):
        for k, v in live["webcast_stream_urls"].items():
            if isinstance(v, str):
                stream_urls[k] = v
            elif isinstance(v, dict) and v.get("main"):
                stream_urls[k] = v["main"]
    if live.get("stream_url_full") and isinstance(live["stream_url_full"], dict):
        sf = live["stream_url_full"]
        if sf.get("flv_pull_url") and isinstance(sf["flv_pull_url"], dict):
            for q, url in sf["flv_pull_url"].items():
                stream_urls[f"flv_{q}"] = url
        if sf.get("hls_pull_url") and isinstance(sf["hls_pull_url"], dict):
            for q, url in sf["hls_pull_url"].items():
                stream_urls[f"hls_{q}"] = url
        if sf.get("rtmp_pull_url") and isinstance(sf["rtmp_pull_url"], dict):
            for q, url in sf["rtmp_pull_url"].items():
                stream_urls[f"rtmp_{q}"] = url

    if not stream_urls:
        result.stream_quality_analysis = {"available": False, "reason": "No stream URLs found"}
        result.raw_keys.append("stream_quality_empty")
        return
    ...
    for format_name, url in stream_urls.items():
        ...
        # نوع البروتوكول
        if "flv" in format_name.lower() or url.endswith(".flv"):
            protocol = "FLV"
            protocols.add("FLV")
        elif "m3u8" in url or "hls" in format_name.lower():
            protocol = "HLS"
            protocols.add("HLS")
        elif "rtmp" in format_name.lower():
            protocol = "RTMP"
            protocols.add("RTMP")
        else:
            protocol = "unknown"
        ...
        # الجودة
        if "HD1" in format_name or "_hd" in url:
            quality = "HD1 (720p+)"
            qualities.add("HD1")
        elif "SD1" in format_name or "_sd" in url:
            quality = "SD1 (480p)"
            qualities.add("SD1")
        elif "uhd" in format_name.lower() or "4k" in format_name.lower():
            quality = "UHD (4K)"
            qualities.add("UHD")
        else:
            quality = "unknown"
        ...
        try:
            params = parse_qs(urlparse(url).query)
            for k in ("expire", "expires", "expiration"):
                if k in params:
                    val = params[k][0]
                    if val.isdigit():
                        ts = int(val)
                        if ts > 1_000_000_000:
                            if earliest_expiry is None or ts < earliest_expiry:
                                earliest_expiry = ts
        except Exception:
            pass
    ...
    # تحديد الأفضل للتسجيل
    best_for_recording = None
    if "HLS" in protocols:
        best_for_recording = "HLS (m3u8) — أفضل جودة وأكثر استقراراً"
    elif "FLV" in protocols:
        best_for_recording = "FLV — أسرع للبث المباشر"
    elif "RTMP" in protocols:
        best_for_recording = "RTMP — بروتوكول كلاسيكي"

    # FFmpeg command
    ffmpeg_cmd = None
    if best_for_recording and earliest_expiry:
        # اختر أول رابط HLS أو FLV
        for url_data in analyzed_urls:
            if "HLS" in url_data["protocol"] or "FLV" in url_data["protocol"]:
                # نحتاج الرابط الفعلي - نأخذه من stream_urls
                for fmt, u in stream_urls.items():
                    if fmt == url_data["format"]:
                        ffmpeg_cmd = f'ffmpeg -i "{u}" -c copy output_{fmt}.mp4'
                        break
                break
    ...
    if len(qualities) >= 2:
        ...
        stability_score += 10
    if earliest_expiry:
        minutes_left = max(0, (earliest_expiry - int(time.time())) // 60)
        if minutes_left > 60:
            stability_score += 10
    ...
    result.stream_quality_analysis = {
        "available": True,
        "total_formats": len(analyzed_urls),
        "protocols_available": sorted(protocols),
        "qualities_available": sorted(qualities),
        "cdn_hosts": sorted(cdn_hosts),
        "cdn_count": len(cdn_hosts),
        "formats_detail": analyzed_urls,
        "best_for_recording": best_for_recording,
        "ffmpeg_command": ffmpeg_cmd,
        "earliest_expiry": earliest_expiry,
        "earliest_expiry_iso": (
            datetime.utcfromtimestamp(earliest_expiry).isoformat() + "Z"
            if earliest_expiry else None
        ),
        "minutes_until_expiry": max(0, (earliest_expiry - int(time.time())) // 60) if earliest_expiry else None,
    ...
    result.raw_keys.append("stream_quality_analysis")
    logger.info(f"🎬 Stream quality: {len(protocols)} protocols, {len(cdn_hosts)} CDNs, "
                f"stability={stability_score}/100")


def _analyze_linkmic(result: ExtractionResult) -> None:
    """يحلل بث اللايف ميك (Multi-Guest / Battle).

    يستخرج:
      - هل البث متعدد الضيوف؟ (with_linkmic)
      - نوع البث المتعدد (multi_live_enum: 0=none, 1=multi-guest, 2=battle)
      - عدد الضيوف الحاليين
      - بيانات كل ضيف (إن وُجدت)
      - هل يمكن الانضمام كضيف؟
      - فرص التفاعل
    """
    live = result.live or {}
    webcast = result.webcast_full_data or {}
    linkmic_data = live.get("link_mic") or webcast.get("link_mic") or {}

    if not linkmic_data:
        result.linkmic_analysis = {
            "available": False,
            "with_linkmic": False,
            "multi_live_enum": 0,
            "type": "solo_broadcast",
            "note": "This is a solo live stream (no multi-guest)",
        }
        result.raw_keys.append("linkmic_analysis_empty")
        return

    multi_live_enum = _safe_int(linkmic_data.get("multi_live_enum"))
    with_linkmic = bool(linkmic_data.get("with_linkmic") or multi_live_enum > 0)

    # تحديد نوع البث
    if multi_live_enum == 0:
        broadcast_type = "solo"
        type_description = "بث فردي — لا يوجد ضيوف"
    elif multi_live_enum == 1:
        broadcast_type = "multi_guest"
        type_description = "بث متعدد الضيوف — يمكن للضيوف الانضمام"
    elif multi_live_enum == 2:
        broadcast_type = "battle"
        type_description = "معركة بث — منافسة بين مذيعين"
    else:
        broadcast_type = "unknown"
        type_description = f"نوع غير معروف (enum={multi_live_enum})"

    # استخراج بيانات الضيوف إن وُجدت
    guests = []
    creators = live.get("creators") or []
        ...
        for c in creators:
            if isinstance(c, dict):
                ...
                guests.append({
                    ...
                    "avatar": (user.get("avatar_urls") or [None])[0] if user.get("avatar_urls") else None,
                    "linkmic_id": c.get("link_mic_id"),
                    "is_anchor": c.get("is_anchor") or user.get("is_anchor"),
                })

    # فرص الانضمام
    can_join = with_linkmic and multi_live_enum == 1
    join_strategy = None
    if can_join:
        join_strategy = (
            "يمكنك الانضمام كضيف — استخدم /api/interact مع action=enter_live_room "
            "ثم انتظر دعوة المذيع أو اطلب الانضمام"
        )

    result.linkmic_analysis = {
        "available": True,
        "with_linkmic": with_linkmic,
        "multi_live_enum": multi_live_enum,
        "broadcast_type": broadcast_type,
        "type_description": type_description,
        "guests_count": len(guests),
        "guests": guests[:10],  # أول 10 ضيوف
        "can_join_as_guest": can_join,
        "join_strategy": join_strategy,
        "strategic_value": (
            "high — بث تفاعلي متعدد الضيوف يجذب جمهوراً أكبر"
            if multi_live_enum == 1
            else "very_high — معركة بث تجذب تفاعلاً ضخماً"
            if multi_live_enum == 2
            else "medium — بث فردي"
        ),
    }
    result.raw_keys.append("linkmic_analysis")
    logger.info(f"🎙️ LinkMic: type={broadcast_type}, guests={len(guests)}, "
                f"can_join={can_join}")


def _decode_ttwid(result: ExtractionResult) -> None:
    """يفك تشفير ttwid إلى مكوناته الثلاثة.

    ttwid format: 1%7C<device_id>%7C<timestamp>%7C<signature>
    أو: 1|<device_id>|<timestamp>|<signature>

    يستخرج:
      - version (عادة 1)
      - device_id (معرف الجهاز الفريد)
      - creation_timestamp (وقت إنشاء التوكن)
      - creation_datetime (ISO 8601)
      - signature (توقيع SHA-256 للتحقق)
      - age_minutes (عمر التوكن بالدقائق)
    """
    ttwid = result.all_ids.get("ttwid") or (result.security_credentials or {}).get("ttwid")
    if not ttwid:
        result.ttwid_decoded = {"available": False, "reason": "No ttwid in extracted data"}
        result.raw_keys.append("ttwid_decoded_empty")
        return

    # URL decode
    from urllib.parse import unquote
    decoded = unquote(ttwid)

    # تقسيم بـ | (بعد URL decode)
    parts = decoded.split("|")
    if len(parts) < 4:
        result.ttwid_decoded = {
            "available": True,
            "raw": ttwid[:100],
            "decoded": decoded[:100],
            "format": "unknown",
        ...
        result.raw_keys.append("ttwid_decoded")
        return

    version, device_id, timestamp_str, signature = parts[0], parts[1], parts[2], parts[3]

    try:
        timestamp = int(timestamp_str)
        creation_datetime = datetime.utcfromtimestamp(timestamp).isoformat() + "Z"
    ...
    if age_minutes is not None:
        if age_minutes < 60:
            validity = "fresh (< 1 hour)"
        elif age_minutes < 1440:  # 24 hours
            validity = "valid (< 24 hours)"
        elif age_minutes < 43200:  # 30 days
            validity = "aging (> 24 hours)"
        else:
            validity = "stale (> 30 days) — may need refresh"
    ...
    result.ttwid_decoded = {
        ...
        "version": version,
        "device_id": device_id,
        "device_id_prefix": device_id[:20] + "..." if len(device_id) > 20 else device_id,
        "creation_timestamp": timestamp,
        "creation_datetime": creation_datetime,
        "signature": signature[:30] + "..." if len(signature) > 30 else signature,
        "signature_length": len(signature),
        "age_minutes": age_minutes,
        "age_hours": age_hours,
        "age_days": age_days,
        "validity": validity,
        "usage_note": "ttwid is required for all TikTok API requests. Use it in Cookie header.",
        "usage_example": f'Cookie: ttwid={ttwid}; sessionid=<your_sessionid>',
    }
    result.raw_keys.append("ttwid_decoded")
    logger.info(f"🔑 ttwid decoded: device={device_id[:15]}..., "
                f"age={age_minutes}min, validity={validity}")


def _calculate_conversion_rates(result: ExtractionResult) -> None:
    """يحسب معدلات التحويل الفعلية من بيانات البث.

    يحسب:
      - conversion_rate_followers: follow_count / enter_count (معدل تحويل المتابعين)
      - conversion_rate_viewers: viewer_count / enter_count (معدل البقاء)
      - drop_off_rate: (enter_count - viewer_count) / enter_count (معدل الترك)
      - replay_ratio: replay_viewers / enter_count (نسبة مشاهدات الإعادة)
      - engagement_per_viewer: (likes + comments + shares) / viewer_count
      - live_efficiency_score: 0-100 (مؤشر كفاءة البث)
    """
    live = result.live or {}
    ...
    enter_count = _safe_int(live.get("enter_count"))
    viewer_count = _safe_int(live.get("viewer_count"))
    follow_count = _safe_int(live.get("room_follow_count") or live.get("follow_count"))
    replay_viewers = _safe_int(live.get("replay_viewers"))
    like_count = _safe_int(live.get("like_count"))
    comment_count = _safe_int(live.get("comment_count") or stats.comment_count)
    share_count = _safe_int(live.get("share_count") or stats.share_count)

    if enter_count == 0:
        result.conversion_analysis = {"available": False, "reason": "No enter_count data"}
        result.raw_keys.append("conversion_analysis_empty")
        return

    # المعدلات
    conversion_rate_followers = _safe_div(follow_count, enter_count)
    conversion_rate_viewers = _safe_div(viewer_count, enter_count)
    drop_off_rate = _safe_div(enter_count - viewer_count, enter_count) if viewer_count <= enter_count else 0
    replay_ratio = _safe_div(replay_viewers, enter_count)
    engagement_per_viewer = _safe_div(like_count + comment_count + share_count, max(viewer_count, 1))

    # مؤشر كفاءة البث (0-100)
    efficiency_score = 0
    if conversion_rate_followers > 0.05:  # 5%+
        efficiency_score += 30
    elif conversion_rate_followers > 0.02:  # 2%+
        efficiency_score += 20
    elif conversion_rate_followers > 0:
        efficiency_score += 10

    if conversion_rate_viewers > 0.5:  # 50%+ retained
        efficiency_score += 30
    elif conversion_rate_viewers > 0.2:
        efficiency_score += 20
    elif conversion_rate_viewers > 0:
        efficiency_score += 10

    if engagement_per_viewer > 0.5:
        efficiency_score += 25
    elif engagement_per_viewer > 0.1:
        efficiency_score += 15
    elif engagement_per_viewer > 0:
        efficiency_score += 5

    if replay_ratio > 0.5:  # الكثير يشاهد الإعادة = محتوى جيد
        efficiency_score += 15
    elif replay_ratio > 0:
        efficiency_score += 5
    ...
    if efficiency_score >= 80:
        grade = "A+ (ممتاز)"
        recommendation = "بث عالي الكفاءة — استمر في هذا النوع من المحتوى"
    elif efficiency_score >= 60:
        grade = "A (جيد جداً)"
        recommendation = "بث جيد — جرّب زيادة التفاعل مع المشاهدين"
    elif efficiency_score >= 40:
        grade = "B (جيد)"
        recommendation = "بث متوسط — حسّن معدل التحويل بطرح أسئلة أو تحديات"
    elif efficiency_score >= 20:
        grade = "C (مقبول)"
        recommendation = "بث ضعيف التحويل — راجع جودة المحتوى والتوقيت"
    else:
        grade = "D (ضعيف)"
        recommendation = "بث ضعيف — أعد التفكير في استراتيجية المحتوى"
    ...
    result.conversion_analysis = {
        ...
        "rates": {
            "follower_conversion_rate": round(conversion_rate_followers, 4),
            "viewer_conversion_rate": round(conversion_rate_viewers, 4),
            "drop_off_rate": round(drop_off_rate, 4),
            "replay_ratio": round(replay_ratio, 4),
            "engagement_per_viewer": round(engagement_per_viewer, 4),
        },
        "efficiency_score": efficiency_score,
        "efficiency_grade": grade,
        "recommendation": recommendation,
        "benchmark": {
            "avg_follower_conversion": 0.02,  # 2% متوسط
            "avg_drop_off": 0.70,  # 70% يغادرون عادة
            "above_avg_conversion": conversion_rate_followers > 0.02,
            "below_avg_drop_off": drop_off_rate < 0.70,
        },
    }
    result.raw_keys.append("conversion_analysis")
    logger.info(f"📊 Conversion: follower_rate={conversion_rate_followers:.4f}, "
                f"drop_off={drop_off_rate:.4f}, efficiency={efficiency_score}/100 ({grade})")


def _analyze_owner_badges(result: ExtractionResult) -> None:
    """يحلل شارات المذيع (owner badges) ومستواه.

    يستخرج من webcast_full_data.owner:
      - badge_list: قائمة الشارات مع مستوياتها
      - privilege_id: معرّف الامتياز
      - level: مستوى المذيع
      - background_color_code: لون الشارة
      - subscription_score
    """
    webcast = result.webcast_full_data or {}
    owner = webcast.get("owner") or {}

    badge_list = owner.get("badge_list") or []
    if not badge_list:
        result.badge_analysis = {"available": False, "reason": "No badge data for owner"}
        result.raw_keys.append("badge_analysis_empty")
        return
    ...
    for b in badge_list:
        if isinstance(b, dict):
            level = _safe_int(b.get("level"))
            max_level = max(max_level, level)
            badges_parsed.append({
                "type": b.get("type") or b.get("badge_type"),
                "level": level,
                "privilege_id": b.get("privilege_id"),
                "background_color": b.get("background_color_code") or b.get("background"),
                "display_badge": b.get("display_badge") or b.get("text"),
                "icon_url": b.get("icon_url") or b.get("image"),
            })

    # تصنيف مستوى المذيع
    if max_level >= 50:
        tier = "elite_broadcaster"
        tier_description = "مذيع نخبة — مستوى عالٍ جداً"
    elif max_level >= 30:
        tier = "veteran_broadcaster"
        tier_description = "مذيع متمرس — مستوى عالٍ"
    elif max_level >= 20:
        tier = "experienced_broadcaster"
        tier_description = "مذيع ذو خبرة"
    elif max_level >= 10:
        tier = "regular_broadcaster"
        tier_description = "مذيع منتظم"
    elif max_level >= 1:
        tier = "novice_broadcaster"
        tier_description = "مذيع مبتدئ"
    else:
        tier = "no_badge"
        tier_description = "لا توجد شارة"

    # subscription_score
    sub_score = owner.get("subscription_score") or owner.get("sub_score")

    result.badge_analysis = {
        "available": True,
        "owner_user_id": owner.get("user_id") or owner.get("id_str"),
        "owner_unique_id": owner.get("unique_id"),
        "badges": badges_parsed,
        "max_badge_level": max_level,
        "broadcaster_tier": tier,
        "tier_description": tier_description,
        "subscription_score": sub_score,
        "total_badges": len(badges_parsed),
        "privilege_features": _get_privilege_features(max_level),
    }
    result.raw_keys.append("badge_analysis")
    logger.info(f"🏅 Badges: {len(badges_parsed)} badges, max_level={max_level}, tier={tier}")


def _get_privilege_features(level: int) -> List[str]:
    """يُرجع ميزات الامتيازات لمستوى معيّن."""
    features = []
    if level >= 5:
        features.append("فلاتر جمال أساسية")
    if level >= 10:
        features.append("حماية من السبام المتقدمة")
    if level >= 15:
        features.append("أدوات تحليل الجمهور")
    if level >= 20:
        features.append("أولوية في الاكتشاف (Discovery)")
    if level >= 25:
        features.append("شارة ذهبية للمذيع")
    if level >= 30:
        features.append("أدوات إدارة البث المتقدمة")
        features.append("فلتر ذكي للمعلقات")
    if level >= 40:
        features.append("دعم متعدد الكاميرات")
        features.append("تحليلات الجمهور المتقدمة")
    if level >= 50:
        features.append("بث بدقة 4K")
        features.append("دعم فني مخصص")
        features.append("شارة نخبة"
        )
    return features
...
def _persist_user_data(result: ExtractionResult) -> None:
    ...
    if owner.unique_id or owner.user_id:
        ...
        # ─── v4.5: رفع مباشر إلى GitHub للحفظ الدائم ───
        if GITHUB_SYNC_ENABLED and os.environ.get("GH_TOKEN"):
            try:
                gh_result = _upload_user_to_github(unique_id, record)
                if gh_result.get("success"):
                    logger.info(f"☁️  Uploaded {unique_id} to GitHub: {gh_result.get('action')}")
                else:
                    logger.warning(f"GitHub upload failed for {unique_id}: {gh_result.get('error', '')[:100]}")
            except Exception as e:
                logger.warning(f"GitHub upload exception for {unique_id}: {e}")
    ...
    for fan in (result.donor_rankings or []):
        ...
        # ─── v4.5: رفع الداعم إلى GitHub أيضاً ───
        if GITHUB_SYNC_ENABLED and os.environ.get("GH_TOKEN"):
            try:
                gh_result = _upload_user_to_github(fan_uid, fan_record)
                if gh_result.get("success"):
                    logger.debug(f"☁️  Uploaded fan {fan_uid} to GitHub: {gh_result.get('action')}")
            except Exception as e:
                logger.debug(f"GitHub upload failed for fan {fan_uid}: {e}")
...
def _upload_user_to_github(unique_id: str, record: Dict[str, Any]) -> Dict[str, Any]:
    """يرفع سجل مستخدم واحد مباشرةً إلى GitHub repo (data/users/<unique_id>.json).

    يستخدم GitHub Contents API لرفع/تحديث الملف.
    Returns: قاموس يحتوي success, sha, commit_url أو error.
    """
    gh_token = os.environ.get("GH_TOKEN")
    if not gh_token:
    ...
    file_path_in_repo = f"data/users/{safe_uid}.json"
    ...
    try:
        ...
        # اقرأ السجل الحالي من GitHub لدمجه (بدلاً من الكتابة فوقه)
        if existing_sha and check_resp.status_code == 200:
            try:
                existing_content_b64 = check_resp.json().get("content", "")
                if existing_content_b64:
                    existing_bytes = base64.b64decode(existing_content_b64)
                    existing_record = json.loads(existing_bytes.decode("utf-8"))
                    # ادمج snapshots و stream_history و top_fans_seen من القديم
                    if isinstance(existing_record, dict):
                        for key in ("snapshots", "stream_history", "top_fans_seen",
                                    "stats_history", "donations_seen"):
                            old_list = existing_record.get(key, [])
                            new_list = record.get(key, [])
                            if old_list and new_list:
                                # ادمج وأزّل التكرار بناءً على timestamp إن أمكن
                                record[key] = (old_list + new_list)[-1000:]
                        # احتفظ بـ first_seen من القديم إن وُجد
                        if existing_record.get("first_seen"):
                            record["first_seen"] = existing_record["first_seen"]
            except Exception as e:
                logger.debug(f"merge failed for {unique_id}: {e}")

        content_bytes = json.dumps(record, ensure_ascii=False, indent=2, default=str).encode("utf-8")
        content_b64 = base64.b64encode(content_bytes).decode("ascii")

        payload = {
            "message": f"data: update {unique_id} (auto-persist v4.5)",
            "content": content_b64,
            "branch": "main",
        }
        if existing_sha:
        ...
        if put_resp.status_code in (200, 201):
            ...
            return {"success": False, "error": f"GitHub PUT {put_resp.status_code}: {put_resp.text[:200]}"}
    except Exception as e:
...
def list_github_users() -> Dict[str, Any]:
    ...
    Returns: قاموس يحتوي success, total_users, users[].
    """
    gh_token = os.environ.get("GH_TOKEN")
    if not gh_token:
        return {"success": False, "error": "GH_TOKEN not set"}
    ...
    repo_owner = os.environ.get("GITHUB_REPO_OWNER", "abuhoney")
    repo_name = os.environ.get("GITHUB_REPO_NAME", "tiktok-extractor-pro")
    api_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/contents/data/users"
    ...
    try:
        ...
        if resp.status_code == 404:
            return {"success": True, "total_users": 0, "users": [],
                    "message": "data/users/ directory not yet created"}
        if resp.status_code != 200:
            return {"success": False, "error": f"GitHub GET {resp.status_code}"}
        ...
        for item in items:
            ...
            users.append({
                ...
                "last_modified": item.get("git_url"),
                "sha": item.get("sha"),
...
def save_sessionid(unique_id: str, sessionid: str, extra_cookies: Dict[str, str] = None) -> Dict[str, Any]:
    """يحفظ sessionid (وكوكيز إضافية) لمستخدم في GitHub repo.

    يُخزَّن في: data/sessions/<unique_id>.json (مجلد منفصل عن data/users/)
    لضمان عدم اختلاط بيانات الجلسة ببيانات المستخدم العامة.

    Args:
        unique_id: معرّف المستخدم (مثل 'abuhoney77')
        sessionid: قيمة sessionid من كوكيز TikTok
        extra_cookies: كوكيز إضافية (ttwid, msToken, sid_tt, etc.)

    Returns:
        قاموس نجاح/فشل مع commit_url
    """
    gh_token = os.environ.get("GH_TOKEN")
    if not gh_token:
        return {"success": False, "error": "GH_TOKEN not set in environment"}

    repo_owner = os.environ.get("GITHUB_REPO_OWNER", "abuhoney")
    repo_name = os.environ.get("GITHUB_REPO_NAME", "tiktok-extractor-pro")
    safe_uid = re.sub(r'[^a-zA-Z0-9_\.\-]', '_', str(unique_id))
    file_path_in_repo = f"data/sessions/{safe_uid}.json"
    api_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/contents/{file_path_in_repo}"
    ...
    session_record = {
        ...
        "note": "Saved via APK — user manually provided sessionid from browser cookies",
    }
    ...
    try:
        ...
        content_bytes = json.dumps(session_record, ensure_ascii=False, indent=2).encode("utf-8")
        content_b64 = base64.b64encode(content_bytes).decode("ascii")

        payload = {
            "message": f"session: save {unique_id} (manual APK v4.5)",
            "content": content_b64,
            "branch": "main",
        }
        if existing_sha:
        ...
        if put_resp.status_code in (200, 201):
            ...
            return {
                ...
                "commit_url": commit.get("html_url"),
                "file_path": file_path_in_repo,
                "saved_at": session_record["saved_at"],
            }
        else:
            return {"success": False, "error": f"GitHub PUT {put_resp.status_code}: {put_resp.text[:200]}"}
    except Exception as e:
...
def get_sessionid(unique_id: str) -> Dict[str, Any]:
    """يقرأ sessionid (وكوكيز إضافية) لمستخدم من GitHub repo.

    Returns: قاموس يحتوي success, sessionid, extra_cookies, saved_at
    """
    gh_token = os.environ.get("GH_TOKEN")
    if not gh_token:
        return {"success": False, "error": "GH_TOKEN not set"}
    ...
    try:
        ...
        if resp.status_code == 404:
            return {"success": False, "error": "No session saved for this user"}
        if resp.status_code != 200:
            return {"success": False, "error": f"GitHub GET {resp.status_code}"}
        ...
        session_record = json.loads(content_bytes.decode("utf-8"))

        return {
            "success": True,
            "unique_id": session_record.get("unique_id"),
            "sessionid": session_record.get("sessionid"),
            "extra_cookies": session_record.get("extra_cookies", {}),
            "saved_at": session_record.get("saved_at"),
            "file_path": file_path_in_repo,
...
def _fetch_tiktok_user_info_from_cookies(sessionid: str, extra_cookies: Dict[str, str] = None) -> Dict[str, Any]:
    """يستدعي TikTok API لجلب بيانات المستخدم الحقيقي باستخدام sessionid الملتقط.

    يستخدم endpoint: https://www.tiktok.com/passport/account/info/ أو
                     https://web.tiktok.com/api/user/detail/

    Returns:
        {
            "unique_id": "abuhoney77",  ← الاسم الحقيقي للمستخدم
            "user_id": "712369...",
            "nickname": "...",
            "avatar": "...",
            "follower_count": N,
    ...
    extra_cookies = extra_cookies or {}
    # بناء cookie header
    cookie_parts = [f"sessionid={sessionid}"]
    ...
    # محاولة 1: passport/account/info (يحتاج فقط sessionid)
    try:
        resp = requests.get(
            "https://www.tiktok.com/passport/account/info/",
            headers=headers,
            timeout=15,
            verify=False,
        )
        if resp.status_code == 200:
            ...
            if data.get("data") and data["data"].get("user"):
                user = data["data"]["user"]
                unique_id = user.get("username") or user.get("unique_id") or user.get("screen_name")
                if unique_id:
                    return {
                        "success": True,
                        "unique_id": unique_id,
                        "user_id": str(user.get("uid") or user.get("user_id") or ""),
                        "nickname": user.get("nickname") or user.get("screen_name"),
                        "avatar": user.get("avatar_url") or user.get("avatar"),
                        "follower_count": user.get("follower_count"),
                        "verified": bool(user.get("verified") or user.get("is_verified")),
                        "source": "passport/account/info",
                    }
    except Exception as e:
        logger.debug(f"passport/account/info failed: {e}")

    # محاولة 2: web.tiktok.com/api/user/me (يحتاج ttwid إضافي)
    try:
        resp = requests.get(
            "https://web.tiktok.com/api/user/me/",
            headers=headers,
            timeout=15,
            verify=False,
        )
        if resp.status_code == 200:
            data = resp.json()
            user = data.get("user") or data.get("userInfo") or data
            unique_id = user.get("unique_id") or user.get("username")
            if unique_id:
                return {
                    "success": True,
                    "unique_id": unique_id,
                    ...
                    "nickname": user.get("nickname"),
                    "avatar": user.get("avatar_url") or user.get("avatar_thumb", {}).get("url_list", [None])[0] if isinstance(user.get("avatar_thumb"), dict) else None,
                    "follower_count": user.get("follower_count"),
                    "verified": bool(user.get("verified")),
                    "source": "web.tiktok.com/api/user/me",
                }
    except Exception as e:
        logger.debug(f"web.tiktok.com/api/user/me failed: {e}")

    # محاولة 3: استخراج من sid_tt cookie (آخر لجوء)
    sid_tt = extra_cookies.get("sid_tt") or extra_cookies.get("sid_guard")
    if sid_tt and "_" in str(sid_tt):
        # sid_tt غالباً يحتوي على user_id
        try:
            parts = str(sid_tt).split("_")
            if len(parts) >= 2 and parts[0].isdigit():
                return {
                    "success": True,
                    "unique_id": f"user_{parts[0]}",
                    "user_id": parts[0],
                    "nickname": None,
                    "avatar": None,
                    "follower_count": None,
                    "verified": False,
                    "source": "sid_tt_cookie_fallback",
                }
        except Exception:
            pass

    return {"success": False, "error": "Could not fetch user info from TikTok with captured sessionid"}


def capture_session_from_cookies(sessionid: str, extra_cookies: Dict[str, str] = None) -> Dict[str, Any]:
    """يُلتقط جلسة كاملة من الكوكيز ويحدّد unique_id تلقائياً.

    الخطوات:
      1. يستقبل sessionid + الكوكيز الإضافية
      2. يستدعي TikTok API لجلب unique_id الحقيقي للمستخدم
      3. يحفظ الجلسة في GitHub (أو محلياً إن لم يتوفر GH_TOKEN)
      4. يُرجع نتيجة كاملة

    هذا المعالج لا يطلب من المستخدم إدخال unique_id — يلتقطه تلقائياً.
    """
    if not sessionid:
    ...
    # ── 1) جلب unique_id الحقيقي من TikTok ─────────────────────────────────────
    user_info = _fetch_tiktok_user_info_from_cookies(sessionid, extra_cookies)
    unique_id = None
    user_meta = {}
    if user_info.get("success"):
        ...
        user_meta = {
            ...
            "info_source": user_info.get("source"),
        }
        logger.info(f"✓ Captured unique_id from TikTok: {unique_id} (via {user_info.get('source')})")
    else:
        # fallback: استخدم "me" كـ unique_id مؤقت
        unique_id = "me"
        user_meta = {"info_source": "fallback_unknown"}
        logger.warning("Could not fetch user info, using 'me' as unique_id")

    # ── 2) حفظ الجلسة ─────────────────────────────────────────────────────────────
    save_result = save_sessionid(unique_id, sessionid, extra_cookies)

    # أضف بيانات المستخدم للنتيجة
    if save_result.get("success"):
        save_result["unique_id"] = unique_id
        save_result["user_info"] = user_meta
        save_result["captured_at"] = datetime.utcnow().isoformat() + "Z"
        save_result["cookies_count"] = 1 + len([v for v in extra_cookies.values() if v])
    else:
        # fallback محلي: احفظ في /tmp وأرجع رابط تنزيل
        try:
            local_dir = "/tmp/tiktok_sessions_local"
            os.makedirs(local_dir, exist_ok=True)
            safe_uid = re.sub(r'[^a-zA-Z0-9_\.\-]', '_', unique_id)
            ...
            session_record = {
                ...
                "user_info": user_meta,
                "note": "Saved locally (GH_TOKEN not set on server). Use the download URL to retrieve.",
            }
            ...
            return {
                ...
                "unique_id": unique_id,
                "user_info": user_meta,
                "captured_at": session_record["saved_at"],
                "cookies_count": 1 + len([v for v in extra_cookies.values() if v]),
                "saved_to": "local",
                "local_path": local_path,
                "download_url": f"/api/session/local/{safe_uid}/download",
                "warning": "GH_TOKEN not set on Render — saved locally. Set GH_TOKEN env var on Render Dashboard → Environment to enable GitHub sync.",
            }
        except Exception as e:
            return {
                "success": False,
                "error": f"Both GitHub and local save failed: {save_result.get('error')} | local: {e}",
                "unique_id": unique_id,
                "user_info": user_meta,
            }
...
def list_local_sessions() -> Dict[str, Any]:
    """يُرجع قائمة بالجلسات المحفوظة محلياً (في /tmp) عند غياب GH_TOKEN."""
    local_dir = "/tmp/tiktok_sessions_local"
    if not os.path.exists(local_dir):
        return {"success": True, "total": 0, "sessions": [], "dir": local_dir}
    ...
    for fname in sorted(os.listdir(local_dir)):
        ...
        try:
            ...
            sessions.append({
                ...
                "nickname": (record.get("user_info") or {}).get("nickname"),
                "filename": fname,
    ...
    return {"success": True, "total": len(sessions), "sessions": sessions, "dir": local_dir}


def get_local_session_download(unique_id: str) -> Dict[str, Any]:
    """يُرجع محتوى جلسة محفوظة محلياً للتنزيل."""
    local_dir = "/tmp/tiktok_sessions_local"
    safe_uid = re.sub(r'[^a-zA-Z0-9_\.\-]', '_', unique_id)
