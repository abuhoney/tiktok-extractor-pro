...
def info():
    ...
    return jsonify({
        ...
        "v47_processors": [
            "_analyze_top_fans (fans influencer tiers)",
            "_analyze_stream_quality (FLV/HLS/RTMP)",
            "_analyze_linkmic (multi-guest/battle)",
            "_decode_ttwid (device_id + timestamp + signature)",
            "_calculate_conversion_rates (live efficiency)",
            "_analyze_owner_badges (broadcaster level)",
        ],
        "v43_processors": [
            ...
            "capture_session_from_cookies (v4.6)",
        ],
        "endpoints": {
            "GET  /": "Web UI",
            ...
            "POST /api/session": "v4.5 Save sessionid to GitHub (manual)",
            "POST /api/session/capture": "v4.6 Auto-capture session (no unique_id needed)",
            "GET  /api/session?unique_id=<id>": "Read saved session",
            "DELETE /api/session/<unique_id>": "Delete session",
            "GET  /api/session/<unique_id>/status": "Check if session exists",
            "GET  /api/session/local": "v4.6 List locally saved sessions",
            "GET  /api/session/local/<unique_id>/download": "Download local session JSON",
        ...
        "env": {
            ...
            "GH_TOKEN": "✅ configured" if os.environ.get("GH_TOKEN") else "❌ not set — set on Render Dashboard → Environment",
            "GITHUB_SYNC_ENABLED": "true" if os.environ.get("GH_TOKEN") else "fallback to local /tmp",
        },
    })
...
# ────────────────────────────────────────────────────────────────────────────
#  v4.7: Advanced Deep Analytics Endpoint
# ────────────────────────────────────────────────────────────────────────────
@app.route("/api/v47-analytics", methods=["POST", "GET"])
def api_v47_analytics():
    """يُرجع تحليلات v4.7 المتقدمة (top_fans, stream_quality, linkmic, ttwid, conversion, badges).
    ...
    Returns:
        {
            "top_fans_analysis": {...},       # تحليل المعجبين المميزين
            "stream_quality_analysis": {...},  # تحليل روابط FLV/HLS/RTMP
            "linkmic_analysis": {...},         # تحليل البث المتعدد الضيوف
            "ttwid_decoded": {...},            # فك تشفير ttwid
            "conversion_analysis": {...},       # معدلات التحويل
            "badge_analysis": {...},            # شارات المذيع ومستواه
        }
    """
    if request.method == "GET":
        url = request.args.get("url", "").strip()
    ...
    logger.info(f"v4.7 analytics request: {url}")
    try:
        result = extract(url, timeout=60)
    except Exception as e:
        logger.exception("v4.7 analytics crashed")
        return jsonify({"success": False, "error": str(e)}), 500
    ...
    return jsonify({
        ...
        "top_fans_analysis": payload.get("top_fans_analysis", {}),
        "stream_quality_analysis": payload.get("stream_quality_analysis", {}),
        "linkmic_analysis": payload.get("linkmic_analysis", {}),
        "ttwid_decoded": payload.get("ttwid_decoded", {}),
        "conversion_analysis": payload.get("conversion_analysis", {}),
        "badge_analysis": payload.get("badge_analysis", {}),
    })


# ────────────────────────────────────────────────────────────────────────────
#  v4.6: Auto-Capture Session — التقاط الجلسة تلقائياً بدون إدخال unique_id
# ────────────────────────────────────────────────────────────────────────────
@app.route("/api/session/capture", methods=["POST"])
def api_capture_session():
    """يُلتقط الجلسة تلقائياً من الكوكيز بدون طلب unique_id.

    POST body:
        {
            "sessionid": "abc123...",          ← مطلوب
            "extra_cookies": {                  ← اختياري
                "ttwid": "...",
                "msToken": "...",
                "sid_tt": "...",
                "passport_csrf_token": "...",
    ...
    Returns:
        {
            "success": true,
            "unique_id": "abuhoney77",         ← ملتقط تلقائياً من TikTok
            "user_info": { ... },
            "saved_to": "github" | "local",
            "commit_url": "...",               ← إن حُفظ في GitHub
            "download_url": "...",             ← إن حُفظ محلياً
            "warning": "..."                   ← إن وُجد تحذير
        }
    """
    ...
    logger.info(f"Auto-capturing session (sessionid length={len(sessionid)}, extra={len(extra_cookies)} cookies)")

    try:
        result = capture_session_from_cookies(sessionid, extra_cookies)
        status_code = 200 if result.get("success") else 500
        return jsonify(result), status_code
    except Exception as e:
        logger.exception("capture_session crashed")
        return jsonify({"success": False, "error": str(e)}), 500


@app.route("/api/session/local")
def api_list_local_sessions():
    """يُرجع قائمة بالجلسات المحفوظة محلياً (عند غياب GH_TOKEN)."""
...
def api_download_local_session(unique_id: str):
    """يُرجع محتوى جلسة محفوظة محلياً كملف JSON قابل للتنزيل."""
    from extractor import get_local_session_download
    data = get_local_session_download(unique_id)
    if not data.get("success"):
        return jsonify(data), 404

    # أرجع كملف JSON قابل للتنزيل
    response = Response(
        data["content"],
        mimetype="application/json",
        headers={
            "Content-Disposition": f'attachment; filename="session_{unique_id}.json"'
        }
    )
    return response
