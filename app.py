...
# ────────────────────────────────────────────────────────────────────────────
#  v5.0: Deep Data Extraction Endpoints — onlinetiktok.py integration
# ────────────────────────────────────────────────────────────────────────────
...
def api_deep_extract():
    ...
    Returns:
        {
            ...
            "unique_id": "...",
            "live_number": N,
            "live_dir": "data/tiktok_deep_data/<uid>/live(N)",
            "files_saved": [...],
            "webmssdk_downloaded": bool,
            ...
        }
    """
    ...
    if request.method == "GET":
        ...
        data = request.get_json(silent=True) or request.form
        url = (data.get("url") or "").strip()
        html = data.get("html")  # optional pre-fetched HTML

    if not url:
        return jsonify({"success": False, "error": "url is required"}), 400

    logger.info(f"Deep extract request: {url}")
    try:
        result = deep_extract(url, html=html)
        status_code = 200 if result.get("success") else 500
        return jsonify(result), status_code
    except Exception as e:
        logger.exception("deep_extract crashed")
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_deep_user_extractions(unique_id: str):
    """يُرجع قائمة بكل استخراجات مستخدم محدد (live1, live2, ...)."""
    from onlinetiktok import list_user_extractions
    data = list_user_extractions(unique_id)
    if not data.get("success"):
        return jsonify(data), 404
    return jsonify(data)
...
def api_deep_extraction_detail(unique_id: str, live_number: int):
    """يُرجع محتوى ملفات استخراج محدد (page.html, json/, complete_data.json, etc)."""
    from onlinetiktok import get_extraction_files
    data = get_extraction_files(unique_id, live_number)
    if not data.get("success"):
        return jsonify(data), 404
    return jsonify(data)
...
def api_deep_download_file(unique_id: str, live_number: int, filename: str):
    """يُنزّل ملف محدد من استخراج محدد (page.html, webmssdk.js, complete_data.json, etc)."""
...
# ────────────────────────────────────────────────────────────────────────────
#  v5.0: Statictor (Statistics) Endpoints
# ────────────────────────────────────────────────────────────────────────────
@app.route("/api/stats")
def api_stats():
    """يُرجع الإحصائيات الشاملة من statictor.py."""
