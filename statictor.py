...
4. إحصائيات الاستخراج عبر نسخ المشروع
5. إحصائيات البثوث (stream_history)
6. إحصائيات المعجبين (top_fans)

لا يتتبع مستخدمين — يحسب فقط من البيانات المستخرجة الموجودة.
"""
...
def users_db_stats() -> Dict[str, Any]:
    """يحسب إحصائيات data/users/ (المستخدمون المحفوظون عبر الاستخراج)."""
...
def sessions_stats() -> Dict[str, Any]:
    """يحسب إحصائيات data/sessions/ (الجلسات الملتقطة عبر APK)."""
    data_root = os.environ.get("DATA_ROOT", "data")
    ...
    # Local /tmp sessions (fallback when GH_TOKEN missing)
...
def deep_data_stats() -> Dict[str, Any]:
    """يحسب إحصائيات data/tiktok_deep_data/ (البيانات العميقة المستخرجة)."""
    ...
    webmssdk_count = 0
    gift_boxes_count = 0
    gift_highest_level = 0
    users_with_webmssdk = 0
                ...
                for fname in files:
                    ...
                    try:
                        ...
                        if fname == "webmssdk.js":
                            webmssdk_count += 1
                            users_with_webmssdk += 1 if uid not in [None] else 0
                        elif fname == "gift_boxes.json":
                            gift_data = _read_json(fpath)
                            if isinstance(gift_data, dict):
                                gift_boxes_count += gift_data.get("total_boxes", 0)
                                lvl = gift_data.get("highest_level", 0)
                                if lvl and lvl > gift_highest_level:
                                    gift_highest_level = lvl
                    except Exception:
                        continue
    ...
    return {
        ...
        "gift_boxes_total": gift_boxes_count,
        "gift_highest_level": gift_highest_level,
        "avg_extractions_per_user": round(total_extractions / len(users), 1) if users else 0,
    }


# ════════════════════════════════════════════════════════════════════════════
#  4. إحصائيات البثوث (من data/users/<id>.json → stream_history)
# ════════════════════════════════════════════════════════════════════════════

def streams_stats() -> Dict[str, Any]:
    """يحسب إحصائيات البثوث من stream_history لكل مستخدم."""
    data_root = os.environ.get("DATA_ROOT", "data")
    users_dir = os.path.join(data_root, "users")
    if not os.path.exists(users_dir):
    ...
    for fname in os.listdir(users_dir):
        ...
        for stream in record.get("stream_history", []):
            ...
            if started:
                try:
                    hour = datetime.utcfromtimestamp(started).hour
                    streams_by_hour[hour] = streams_by_hour.get(hour, 0) + 1
                except Exception:
                    pass

    peak_hour = max(streams_by_hour, key=streams_by_hour.get) if streams_by_hour else None
    ...
    return {
        ...
        "streams_by_hour": {str(k): v for k, v in sorted(streams_by_hour.items())},
        "avg_viewers_per_stream": round(total_peak_viewers / total_streams, 0) if total_streams else 0,
        "avg_enter_per_stream": round(total_enter_count / total_streams, 0) if total_streams else 0,
    }


# ════════════════════════════════════════════════════════════════════════════
#  5. إحصائيات المعجبين (top_fans_seen)
# ════════════════════════════════════════════════════════════════════════════
...
def fans_stats() -> Dict[str, Any]:
    ...
    for fname in os.listdir(users_dir):
        ...
        for fan in record.get("top_fans_seen", []):
            ...
            amount = _safe_int(fan.get("amount"))
            if amount > 0:
                fans_by_amount.append({"unique_id": uid, "amount": amount})

    # ترتيب أعلى المعجبين تبرعاً
    fans_by_amount.sort(key=lambda x: x["amount"], reverse=True)
    top_donors = fans_by_amount[:10]

    # أكثر المعجبين تكراراً
    most_frequent = sorted(fans_by_unique_id.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    return {
        ...
        "most_frequent_fans": [{"unique_id": uid, "appearances": count} for uid, count in most_frequent],
    }
